...
WITH cte_table AS(
	SELECT MONTH(txn_date) AS txn_month,
		...
		SUM(CASE WHEN(txn_type = "purchase" OR txn_type = "withdrawal")=TRUE THEN 1 ELSE 0 END) AS purchase_or_withdrawal
	FROM customer_transactions
	GROUP BY MONTH(txn_date), customer_id)
SELECT txn_month AS "Month",
	COUNT(customer_id) AS "Customers"
FROM cte_table
WHERE deposits > 1 AND purchase_or_withdrawal >= 1
GROUP BY txn_month
ORDER BY txn_month;
```
...
WITH cte_table AS(
	SELECT customer_id,
		...
		SUM(CASE WHEN(txn_type = "deposit")=TRUE THEN txn_amount ELSE txn_amount*(-1) END) AS transactions
	FROM customer_transactions
	GROUP BY MONTH(txn_date), customer_id
