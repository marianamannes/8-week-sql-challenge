...
```sql
WITH cte_table AS(
		SELECT DISTINCT(e.visit_id) AS visits,
...
```sql
WITH cte_table AS(
		SELECT e.visit_id AS visits,
			SUM(CASE WHEN(ei.event_name = "Page View" AND p.page_name = "Checkout")=TRUE THEN 1 END) AS checkout,
	...
	GROUP BY e.visit_id)
SELECT CONCAT(ROUND((SUM(CASE WHEN(checkout >= 1 AND purchases IS NULL)=TRUE THEN 1 END)/COUNT(DISTINCT (visits)))*100,2),"%") AS "Percentage (All pages)",
	CONCAT(ROUND((SUM(CASE WHEN(checkout >= 1 AND purchases IS NULL)=TRUE THEN 1 END)/SUM(CASE WHEN(checkout >= 1)=TRUE THEN 1 END))*100,2),"%") AS "Percentage (Checkout)"
FROM cte_table;
```
...
cte_table2 AS(
    SELECT DISTINCT(e.visit_id) AS visit_id,
		SUM(CASE WHEN(ei.event_name = "Purchase")=TRUE THEN 1 END) AS purchases
		FROM events e
		INNER JOIN event_identifier ei
		ON e.event_type = ei.event_type
